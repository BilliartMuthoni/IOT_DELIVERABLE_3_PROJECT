...
The full source code is available in this repository: [`sketch.ino`](sketch.ino)

Security note: the InfluxDB API token is not stored in this repository. It is kept in a local `secrets.h` file which is excluded via `.gitignore`; a template is provided as [`secrets_example.h`](secrets_example.h). The token will be rotated after assessment.
...
The complete dashboard definition is provided as [`grafana_dashboard.json`](grafana_dashboard.json) and can be imported directly into any Grafana instance to reproduce the dashboard.
...
*Figure 13: Group members working on the prototype and cloud dashboard setup.*
